...
class Connection extends EventEmitter {
	...
	async connect() {
		try {
			const conn = await odbc.connect(this.config.connectionString);

			debug(`connection (${ this.uuid }): opened`);
			this.connection = conn;
			this.emit('connect', errors.formatDriverError(null));
		} catch (error) {
			this.emit('connect', errors.formatDriverError(error));
		}
